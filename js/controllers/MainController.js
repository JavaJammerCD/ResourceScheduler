angular.module('resourceApp').controller('MainController', function ($scope) {
    ...
    var getMyBookings = function(employeeID){
        ...
        for(var s in deskInfo){            
            if(deskInfo[s].indexOf(userName) != -1 ){
                myDesks[counter] = deskInfo[s];
                var date, startTime, endTime, deskID;
                var properties = deskInfo[s].split(",");
                for(var x in properties){
                    console.log(properties[x]);
                    if(properties[x].indexOf("date") > 0 ){
                        date = properties[x];
                        continue;
                    } 
                    if(properties[x].indexOf("start_time" > 0 )){
                        startTime = properties[x];
                        continue;
                    } 
                    if(properties[x].indexOf("end_time" > 0 )){
                        endTime = properties[x];
                        continue;
                    } 
                    if(properties[x].indexOf("resourceName" > 0 )){
                        deskID = properties[x];
                        continue;
                    }
                }
                console.log(date + "// " + startTime + "// " + endTime + "// " + deskID);
                counter ++;
            }
        }
